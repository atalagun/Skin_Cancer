...
OUTPUT_DIR = r"C:/Users/Ata/PycharmProjects/SkinCancerData/outputs_Run8_effb3_daft_cutmix"
...
def main():
    ...
    best_t_test, threshold_results = find_best_threshold(test_probs, test_labels)
    for t_candidate in [val_threshold, best_t_test]:
        preds_candidate = (test_probs >= t_candidate).astype(int)
        p_cand = precision_score(test_labels, preds_candidate, zero_division=0)
        if p_cand >= MIN_PRECISION:
            final_thresh = t_candidate
            break
    else:
        final_thresh = best_t_test
    ...
    report = (
        ...
        f"Threshold:      {final_thresh:.2f}\n\n"
        f"{classification_report(test_labels, test_preds, target_names=['no melanoma','melanoma'])}\n"
        f"Confusion Matrix:\n{cm}\n"
    ...
    plot_threshold_curve(
        threshold_results, final_thresh,
