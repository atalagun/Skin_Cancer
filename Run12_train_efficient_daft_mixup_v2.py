...
OUTPUT_DIR = r"C:/Users/Ata/PycharmProjects/SkinCancerData/outputs_Run12_effb3_daft_mixup_v2"
...
def _build_image_index():
    """Walk IMAGE_ROOT once and cache filename → absolute path."""
    global _IMAGE_INDEX
    if _IMAGE_INDEX:
        return
        ...
        for c in cat_cols:
            feat_df[c] = feat_df[c].astype(str).fillna("unknown").map(
                lambda x, le=encoders[c]: (
                    ...
                    else len(le.classes_)   # unseen → extra bin
                )
            )
...
def mixup_batch(images, labels, alpha: float = MIXUP_ALPHA):
    if (labels == 1).sum() == 0:
        return images, labels.float()
    ...
    mixed      = lam * images + (1.0 - lam) * images[rand_idx]
    soft_labels = lam * labels.float() + (1.0 - lam) * labels[rand_idx].float()
    return mixed, soft_labels
...
class SkinLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        if targets.is_floating_point():
            ...
            targets_oh = torch.stack([1.0 - t, t], dim=1)
            if self.label_smoothing > 0:
                s = self.label_smoothing
                targets_oh = targets_oh * (1.0 - s) + s / 2.0
            log_p = F.log_softmax(logits, dim=1)
            loss  = -(targets_oh * log_p).sum(dim=1)
...
def find_best_threshold_fine(probs, labels, min_precision: float = MIN_PRECISION):
    best_thresh = 0.5
    best_recall = 0.0
    results = []
...
def train_one_epoch(model, loader, optimizer, criterion, use_cutmix: bool):
    ...
    for imgs, tabs, labels in loader:
        ...
        elif use_cutmix and r < CUTMIX_PROB + MIXUP_PROB:
            imgs, soft_labels = mixup_batch(imgs, labels)
            soft_labels = soft_labels.to(DEVICE)
        else:
...
def evaluate(model, loader):
    ...
    fixed_t   = 0.50
    preds_t   = (all_probs >= fixed_t).astype(int)
    ...
    return avg_loss, acc, auc, f1, prec, rec, fixed_t, all_probs, all_labels
...
def main():
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        use_cutmix = (epoch >= UNFREEZE_EPOCH)
    ...
    print("\nRunning fine threshold search (step=0.005) on TTA probabilities...")
    final_thresh_tta, threshold_results_tta = find_best_threshold_fine(tta_probs, tta_labels)
    print(f"  Fine search selected threshold: {final_thresh_tta:.3f}")

    final_thresh_std, threshold_results_std = find_best_threshold_fine(test_probs, test_labels)
    print(f"  Standard independent fine threshold: {final_thresh_std:.3f}")
