...
OUTPUT_DIR = r"C:/Users/Ata/PycharmProjects/SkinCancerData/outputs_Run10_effb3_daft_regularised"
...
class EfficientNetDAFT(nn.Module):
    def __init__(self, tab_dim: int, num_classes: int = 2, dropout: float = 0.5):
        super().__init__()
...
def main():
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        if epoch == UNFREEZE_EPOCH:
            ...
            scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
                optimizer, T_0=16, T_mult=1, eta_min=5e-7
            )
