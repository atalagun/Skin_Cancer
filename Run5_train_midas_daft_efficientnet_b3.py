...
OUTPUT_DIR = r"C:/Users/Ata/PycharmProjects/SkinCancerData/outputs_Run5_daft_efficientnet_b3"
os.makedirs(OUTPUT_DIR, exist_ok=True)

MODEL_PATH = os.path.join(OUTPUT_DIR, "best_daft_efficientnet_b3.pth")
...
model = DAFTEfficientNetB3(tabular_input_dim, num_classes).to(DEVICE)
...
for epoch in range(NUM_EPOCHS):
    if epoch == FREEZE_EPOCHS:
        print("\nUnfreezing EfficientNet-B3 backbone for fine-tuning...")
    ...
    if val_f1 > best_val_f1:
        ...
        print("Best EfficientNet DAFT model saved:", MODEL_PATH)
    else:
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("EfficientNet-B3 DAFT Training and Validation Loss")
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(OUTPUT_DIR, "efficientnet_daft_loss_curve.png"), dpi=300, bbox_inches="tight")
plt.show()
...
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
plt.title("EfficientNet-B3 DAFT Training and Validation Accuracy")
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(OUTPUT_DIR, "efficientnet_daft_accuracy_curve.png"), dpi=300, bbox_inches="tight")
plt.show()
...
plt.xlabel("Epoch")
plt.ylabel("Score")
plt.title("EfficientNet-B3 DAFT Validation F1 and AUC")
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(OUTPUT_DIR, "efficientnet_daft_f1_auc_curve.png"), dpi=300, bbox_inches="tight")
plt.show()
...
plt.title("EfficientNet-B3 DAFT Best Threshold per Epoch")
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(OUTPUT_DIR, "efficientnet_daft_threshold_curve.png"), dpi=300, bbox_inches="tight")
plt.show()

print("\nLoading best EfficientNet-B3 DAFT model for final test...")
...
print("\n===== EFFICIENTNET-B3 DAFT TEST RESULTS =====")
...
history_df.to_csv(os.path.join(OUTPUT_DIR, "efficientnet_daft_training_history.csv"), index=False)
...
pred_df.to_csv(os.path.join(OUTPUT_DIR, "efficientnet_daft_test_predictions.csv"), index=False)

with open(os.path.join(OUTPUT_DIR, "efficientnet_daft_test_report.txt"), "w") as f:
    f.write("===== EFFICIENTNET-B3 DAFT TEST RESULTS =====\n")
    ...
    f.write("\n\n")
    f.write("Method:\n")
    f.write("DAFT + EfficientNet-B3\n")
    f.write("Image size: 300x300\n")
