...
OUTPUT_DIR = r"C:/Users/Ata/PycharmProjects/SkinCancerData/outputs_Run3_daft_focal_sampler_threshold"
os.makedirs(OUTPUT_DIR, exist_ok=True)

MODEL_PATH = os.path.join(OUTPUT_DIR, "best_daft_focal_sampler_threshold.pth")
...
for epoch in range(NUM_EPOCHS):
    ...
    if val_f1 > best_val_f1:
        ...
        print("Best DAFT model saved:", MODEL_PATH)
    else:
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("DAFT Training and Validation Loss")
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(OUTPUT_DIR, "daft_loss_curve.png"), dpi=300, bbox_inches="tight")
plt.show()
...
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
plt.title("DAFT Training and Validation Accuracy")
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(OUTPUT_DIR, "daft_accuracy_curve.png"), dpi=300, bbox_inches="tight")
plt.show()
...
plt.xlabel("Epoch")
plt.ylabel("Score")
plt.title("DAFT Validation F1 and AUC")
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(OUTPUT_DIR, "daft_f1_auc_curve.png"), dpi=300, bbox_inches="tight")
plt.show()
...
plt.title("DAFT Best Validation Threshold per Epoch")
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(OUTPUT_DIR, "daft_threshold_curve.png"), dpi=300, bbox_inches="tight")
plt.show()

print("\nLoading best DAFT model for final test...")
...
history_df.to_csv(os.path.join(OUTPUT_DIR, "daft_training_history.csv"), index=False)
...
pred_df.to_csv(os.path.join(OUTPUT_DIR, "daft_test_predictions.csv"), index=False)

with open(os.path.join(OUTPUT_DIR, "daft_test_report.txt"), "w") as f:
    f.write("===== DAFT TEST RESULTS =====\n")
    ...
    f.write("\n\n")
    f.write("Method:\n")
    f.write("DAFT-style DenseNet121\n")
    f.write("Tabular metadata generates gamma/beta affine modulation parameters.\n")
    f.write("Imbalance handling: WeightedRandomSampler + FocalLoss + threshold tuning.\n")
